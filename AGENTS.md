...
You are operating with an andurel framework project. Andurel is a fullstack hypermedia-native web framework designed to build scalable and maintainable applications in Go. 

For a reference on how to use Andurel, run the command 'andurel llm' for an overview.

In this overview you will also find specific commands that details how to work with the different components of the framework such. 

An example would be 'andurel llm models'

## Workflow guidelines

Never run 'go build', 'templ generate' or 'sqlc generate' directly. 

Always use the Andurel CLI to build and run your applications. This ensures that all necessary preprocessing steps are executed.

'andurel views generate' to compile templ files.
'andurel queries compile' to compile sqlc files.

## Frontend Component Library

This project uses basecoat ui which is a HTML+CSS port of shadcn.
