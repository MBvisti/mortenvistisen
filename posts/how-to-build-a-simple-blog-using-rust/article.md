...
So, you've been reading about `rust` for quite some time, learned that it's been the most loved
programming langugage multiple years in a row in Github's annual survey and you want in. But, you're
not a system level programmer and reddit keeps telling you to 'use the right tool for the job' and 'rust is
more aimed at low level stuff like writing databases and programming drone'  (the first is actually good advice,
I just need it to create a dramatic setting). So you get dishearthed and choose to use what you've always been using. 
I think that's a mistake as `rust` is an amazing language both in terms of speed of execution but also developer experience. 
Not many languages gives you a handy pair-programmer, i.e. the compiler, that only gives good advice, and shuts up in the meantime (`rust`'s compiler is amazing).
I've been toying with Rust for some time now and overall it's been a great experience (both learning and developer).
So to get my hands a bit more dirty I decided to re-write my blog using Rust which was previously was built using `Next.js`
(This has also been a thing coming since I'm doing my best to move away from SPAs and Javascript, but that's for another post).

The goal of the re-write, is to get _something_ out which could serve as a learning example and be built upon to further my understanding of the language. 
I hope this post does the same for you.

*Side note*:
I write Go for a living, and have been coding professionally for the last ~5 years, so while I'm confident in my overall skills this tutorial might not always show idomatic Rust and best pratices. 
There're a bunch of much more `rust` abled developers than me who have written some great material, much of which lay the foundation for what I'm about to show you. At the end of the article 
you'll find a resource list that you can checkout after reading (and hopefully coding along) this article. If you're just interested in seeing the code you can find the repo [here](https://github.com/mbvisti/awesome-blog). 

PRs are welcome!

## Kicking things off
So, to build your new blog using Rust, you're going to Rust installed on your system and then run `cargo new awesome-blog`.
...
serde_json = "1.0"
```
Feel free to use another version than the ones listed above; it's whats I used at the time of writing this piece.
...
There are multiple other choices (see for example [warp](insert-warp-link), [rocket](insert-rocket-link), [axum](insert-link-here)) but I throughouly enjoy Actix's API and have built multiple projects using it. Further more, 
it performs really [well](insert-link) and can handle _a lot_ requests.

Next is `serde` which is a library you most likely is familiar with if you've done any Rust programming. If not, it's bascially a set of 
...
you simply implement [the methods](https://serde.rs/custom-serialization.html) for you data structure and you're off to the races.
For this project, `serde` will be used for serializing the meta data of a blog post (also named front matter) from a `.toml` file into a Rust struct.
...
templating so wanted to replicate that experience in my personal projects. In a world where everything is components and the slighest duplication is a
death sin, the experience using "old school" templating has been a relieve.
...
and call a start-up functions. With that requirement, we end up with something like this:
...
Not much going on here, except a bunch of errors, so lets break it down real quick: 
...
- We create a listener that listen on port `8080`
- We pass the listener to the `start_blog` and we're off to the races

We will deal with `start_blog` later, which means that the only thing left that haven't been addressed is `#[axtic_web::main]`.
This is macro, or more precisely, a `proc_macro` which generates some code for us that makes are `awesome_blog` run asynchronously.
The `rust` team made a consionously decision not to provide a async runtime in the standard library but instead opted to provide essentials and let the community do the rest.
The reason(s) why are manyfold, but one reason could be that there are no consensus on what such runtime should [look like](https://www.reddit.com/r/rust/comments/ui7ayd/comment/i7akj6d/?utm_source=share&utm_medium=web2x&context=3).
...
Most of the above code should make sense to you (assuming familiarty with Rust) but lets quickly touch upon the `move` keyword.
...
understandable sentence. What it boils down to are Rust's ownership model and how it deals with memory (de)allocation and references.
...
That would leave dangling references laying around thats not accounted far and thats no bueno in `rust`.
So, to get around that, we tell `App` to take ownership of the values passed to it so that each instance own what is passed to it.

Even though we're going a bit "oldschool" here and using pure simple html we would still like to have some resuability.
And this is exactly where `tera` comes in. However, since the templates will remain the same once we chuck it into prod, it will probably be a good idea only to load them one time and then pass a reference.
...
All good, but one things missing. We need to add a templates directory in the root directory so go 
ahead and `mkdir templates`. We will get back to `main.rs`, `lib.rs` and all that soon but for now 
lets quickly touch upon the templates so we have something to show.

## Dude, where's the components?
I started out with `react` when I got my first job as a frontend developer. Components was all the rage (well, they still 
...
change my mind quite a bit on this but that will be for another article. As you know, we're not going to be doing components here 
...
Let see how that is done, so go ahead and create a new file under `templates` called `home.html`:
...
In that holder, create a file called `mod.rs` and `home_handler.rs`. Lets start simple with some test data to show something when we load the home page. 
...
that's how we do it. We then simple call `.insert()` on the context, provide the variable name and the data and we're good to go.
...
Lastly, before we can access our new shiney handler, we need to make it accessable so open up `mod.rs` under `src/handlers`:
...
While we were setting up our html templates you might have noticed what looks like `tailwindcss` classes and then promptly wondering what the hell kind of end result it had on the home page.
First of, good on you for chosing great frontend tooling. Secondly, we haven't yet included the css needed for our styles to actually do something.
...
Now, we can go about this in a number of ways all depending on what kind of traffic we expect to get and how (over)engineered we are planning to make this.

One option could be to serve the content of `index.css` through an S3 bucket and then either point an url directly at it which then becomes referenced here.
...
So, create two new folders at the root, namely `static` and under that, `css`. Next up we need to do some tailwind setup that you can find on their own site, so for your convinence, here are the `cmd`s you can run in your terminal:
...
Those two scripts basically gives a `cmd` to watch for any changes in development and then build for production when we merge.
...
Lets start with the 2nd itemo the list since we pretty much already did that. To not bore you with the same thing twice, 
...
copy the content, create a new file under `templates` called `post.html`.
...
[fasterthanlime](https://fasterthanli.me/) whom, if you haven't already, should definitely check out. This guys does some *serious* deep dives and really 
knows his craft.
...
If this look suspeciously like our `struct Frontmatter {....}`, then its because it's. 

Next order business, add some logic to extract all the frontmatters we might have and display it on our home page.
Lets start with the logic for getting all the frontmatters of our awesome blogs, so open `home_handler.rs` and add the following:
...
As you might be able to tell, I haven't spend the most time on errors in this function and basically just log whatever error comes from `ignore` and then return
the most similar one from `std::io::ErrorKind`. This can definitely be done better using libraries like `anyerror` or `thiserror`. I encourge you to play around 
...
The last thing that's needed is to add the a handler for the `post.html` page and extract the markdown of the article, 
...
The compiler errors you get can be annjoying, but will also guide you towards the correct path, and compared to something like `Typescript` it's a godsend.
The more you write the easier it becomes.

It has quickly become my go-to language for writing small services as it plays very nice with `aws lambda`s and due how fast it is, cold starts are not even an issue any more.
So, please take what you have here and extend it as much as you want.

# Resources
Here is a collection of all the material I've used to get started with `rust`. Some of it is free and some of it is paid, but all of them is highly recommendable
